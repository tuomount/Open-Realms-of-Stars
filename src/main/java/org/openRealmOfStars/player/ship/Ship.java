...
public class Ship extends Construction {
    ...
    * 
    * @param design
    *            from where actual ship is created
    */
    ...
    *            DataInputStream
    * @throws IOException
    *             if there is any problem with DataInputStream
    */
   public Ship(final DataInputStream dis) throws IOException {
      ...
      hull = ShipHullFactory.createByName(hullName, SpaceRaceUtility.getRaceByIndex(raceIndex));
      image = ShipImage.scaleTo32x32(hull.getImage());
    ...
    *            Data Output Stream
    * @throws IOException
    *             if there is any problem with DataOutputStream
    */
   ...
   public void regenerateShield() {
      ...
      for (int i = 0; i < components.size(); i++) {
         ...
         if (comp.getType() == ShipComponentType.SHIELD && componentIsWorking(i) && !shieldsUp) {
            workingShields = true;
            shieldsUp = true;
         ...
         if (comp.getType() == ShipComponentType.SHIELD_GENERATOR && componentIsWorking(i) && !generatorUp) {
            generatorUp = true;
   ...
   /**
    * Check if certain component has energy or not. Returns true if component
    * has energy. This also checks that component is functional.
    * 
    * @param index
    *            Component index
   ...
   /**
    * Check if certain component has energy and hull points so it is
    * functioning.
    * 
    * @param index
    *            Component index
   ...
   public int getSpeed() {
      for (int i = 0; i < components.size(); i++) {
         ShipComponent comp = components.get(i);
         if (hullPoints[i] > 0 && comp.getType() == ShipComponentType.ENGINE && hasComponentEnergy(i)) {
   ...
   public int getScannerLvl() {
      ...
      for (int i = 0; i < components.size(); i++) {
         ShipComponent comp = components.get(i);
         if (hullPoints[i] > 0 && comp.getType() == ShipComponentType.SCANNER && hasComponentEnergy(i)
               && comp.getScannerRange() > result) {
   ...
   public int getHitChance(final ShipComponent weapon) {
      ...
      for (int i = 0; i < components.size(); i++) {
         ShipComponent comp = components.get(i);
         if (hullPoints[i] > 0 && comp.getType() == ShipComponentType.TARGETING_COMPUTER && hasComponentEnergy(i)) {
    ...
    * 
    * @param damage
    *            Amount damage to cause
    * @param shipDamage
    *            Ship Damage to more description information about damage
    ...
    * @param weapon
    *            ShipComponent
    * @return 1 No damage, not even dent 0 No damage, but shield or armor got
    *         lower -1 Got damage -2 Destroyed
    */
   public ShipDamage damageBy(final ShipComponent weapon) {
      ...
      case WEAPON_PHOTON_TORPEDO: {
         ...
         if (damage > 0) {
            ...
            if (this.getShield() / 2 <= weapon.getDamage() || DiceGenerator.getRandom(99) < chance) {
               this.setShield(this.getShield() - 1);
         ...
         if (damage >= 0) {
            ...
            damage = damage + this.getArmor() / 2;
            if (this.getArmor() / 4 <= damage || DiceGenerator.getRandom(99) < chance) {
               this.setArmor(this.getArmor() - 1);
      ...
      case WEAPON_HE_MISSILE: {
         ...
         if (damage > 0) {
            ...
            if (this.getArmor() / 2 <= weapon.getDamage() || DiceGenerator.getRandom(99) < 5) {
               this.setArmor(this.getArmor() - 1);
               return new ShipDamage(0, "Attack hit the armor!");
            }
         ...
         if (damage >= 0) {
            ...
            damage = damage + this.getShield() / 2;
            if (this.getShield() / 4 <= damage || DiceGenerator.getRandom(99) < 5) {
               this.setShield(this.getShield() - 1);
               return new ShipDamage(0, "Attack hit the shield!");
            }
         }
   ...
   public int getDefenseValue() {
      ...
      for (int i = 0; i < components.size(); i++) {
         ShipComponent comp = components.get(i);
         if (hullPoints[i] > 0 && comp.getType() == ShipComponentType.JAMMER && hasComponentEnergy(i)) {
   ...
   public boolean isColonyShip() {
      for (int i = 0; i < components.size(); i++) {
         ShipComponent comp = components.get(i);
         if (hullPoints[i] > 0 && comp.getType() == ShipComponentType.COLONY_MODULE && hasComponentEnergy(i)
               && getColonist() > 0) {
            return true;
         }
      }
    ...
    * 
    * @return True if ship has functional planetary invasion module, otherwise
    *         false
    */
    ...
    * 
    * @return one of these: GuiStatics.COLOR_GREEN_TEXT
    *         GuiStatics.COLOR_YELLOW_TEXT GuiStatics.COLOR_RED_TEXT
    */
   ...
   /**
    * Get maximum shield for ship. This is very useful information when ship is
    * being design and shown on ship view
    * 
   ...
   /**
    * Get maximum armor for ship. This is very useful information when ship is
    * being design and shown on ship view
    * 
   ...
   public int getTotalMilitaryPower() {
      ...
      for (int i = 0; i < components.size(); i++) {
         ShipComponent comp = components.get(i);
         if ((comp.getType() == ShipComponentType.WEAPON_BEAM || comp.getType() == ShipComponentType.WEAPON_RAILGUN
               || comp.getType() == ShipComponentType.WEAPON_HE_MISSILE
               || comp.getType() == ShipComponentType.WEAPON_PHOTON_TORPEDO) && componentIsWorking(i)) {
            militaryShip = true;
            power = power + comp.getDamage();
         }
         if (comp.getType() == ShipComponentType.WEAPON_ECM_TORPEDO && componentIsWorking(i)) {
            power = power + comp.getDamage() / 2;
         }
         if (comp.getType() == ShipComponentType.ARMOR || comp.getType() == ShipComponentType.SHIELD) {
            power = power + comp.getDefenseValue();
         }
         if (comp.getType() == ShipComponentType.ENGINE && getHull().getHullType() != ShipHullType.STARBASE
               && componentIsWorking(i)) {
            power = power + comp.getTacticSpeed() - 1;
         }
         if (comp.getType() == ShipComponentType.TARGETING_COMPUTER && componentIsWorking(i)) {
    ...
    * 
    * @param flag
    *            Flat to set or disable
    * @param value
    *            True to set and false to disable
    */
